...
namespace Microsoft.ML.OnnxRuntime.Tests
{
    internal static class TestDataLoader
    {
        ...
        static NamedOnnxValue LoadTensorPb(Onnx.TensorProto tensor, string nodeName, NodeMetadata nodeMeta)
        {
            ...
            //            // We can not handle sparse tensors yet down below even if shapes match
            //            if (meta.OnnxValueType == OnnxValueType.ONNX_TYPE_TENSOR)
            //            {
            //                if ((Tensors.TensorElementType)tensor.DataType == meta.ElementDataType &&
            //                    tensor.Dims.Count == meta.Dimensions.Length)
            //                {
            //                    int i = 0;
            ...
            return CreateNamedOnnxValueFromTensor(nodeName, tensor, intDims);
        }

        internal static NamedOnnxValue CreateNamedOnnxValueFromTensor(string nodeName, Onnx.TensorProto tensor, int[] intDims)
        {
            var elementType = (Tensors.TensorElementType)tensor.DataType;
            switch (elementType)
            {
